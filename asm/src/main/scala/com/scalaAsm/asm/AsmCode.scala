...
//    }}
    ...
    def sub[O1, O2](p1: O1, p2: O2)(implicit ev: SUB_2[O1, O2]) = twoOps(p1,p2,ev) 
      
    def add[O1, O2](p1: O1, p2: O2)(implicit ev: ADD_2[O1, O2]) = twoOps(p1,p2,ev)

    def mul[O1](p1: O1)(implicit ev: MUL_1[O1]) = oneOp(p1,ev)
    
    def push[O1](p1: O1)(implicit ev: PUSH_1[O1]) = oneOp(p1,ev)

    def pop[O1](p1: O1)(implicit ev: POP_1[O1]) = oneOp(p1,ev)
      
    def dec[O1](p1: O1)(implicit ev: DEC_1[O1]) = oneOp(p1,ev)

    def and[O1, O2](p1: O1, p2: O2)(implicit ev: AND_2[O1, O2]) = twoOps(p1,p2,ev)

    def not[O1](p1: O1)(implicit ev: NOT_1[O1]) = oneOp(p1,ev)

    def lea[O1, O2](p1: O1, p2: O2)(implicit ev: LEA_2[O1, O2]) = twoOps(p1,p2,ev)

    def mov[O1, O2](p1: O1, p2: O2)(implicit ev: MOV_2[O1, O2]) = twoOps(p1,p2,ev)

    def shr[O1, O2](p1: O1, p2: O2)(implicit ev: SHR_2[O1, O2]) = twoOps(p1,p2,ev)

    def jnz(labelRef: String) = LabelRef(labelRef, 0x75.toByte)
    
    def jnz[O1](p1: O1)(implicit ev: JNZ_1[O1]) = oneOp(p1,ev)

    def jz(labelRef: String) = LabelRef(labelRef, 0x74.toByte)
    
    def jz[O1](p1: O1)(implicit ev: JZ_1[O1]) = oneOp(p1,ev)
    
    def int[O1](p1: O1)(implicit ev: INT_1[O1]) = oneOp(p1,ev)

    def shl[O1, O2](p1: O1, p2: O2)(implicit ev: SHL_2[O1,O2]) = twoOps(p1,p2,ev)

    def sbb[O1, O2](p1: O1, p2: O2)(implicit ev: SBB_2[O1, O2]) = twoOps(p1,p2,ev)
    ...
    def rdrand[O1](p1: O1)(implicit ev: RDRAND_1[O1]) = oneOp(p1,ev)
