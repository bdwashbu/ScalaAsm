...
import com.scalaAsm.x86.Operands.ConstantWriter
  ...
  class ImmFormat extends OneOperandFormat[imm] {
  ...
  class RmFormat extends OneOperandFormat[rm] {
  ...
  class RmImmFormat extends TwoOperandFormat[rm, imm] {
  ...
  class RegRmFormat extends TwoOperandFormat[reg, rm] {
  ...
  class MemRegFormat extends TwoOperandFormat[rm, reg] {

    def getAddressingForm(op1: rm, op2: reg, opcodeExtension: Byte, opcodeSelectsRegister: Boolean) = {
      new RegRmFormat {}.getAddressingForm(op2, op1, opcodeExtension, opcodeSelectsRegister)
    }
  }